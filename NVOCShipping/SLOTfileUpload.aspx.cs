...
{
    public partial class SLOTfileUpload : System.Web.UI.Page
    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            ...
            DataTable dtx = Manag.InsertSLOTExcelUploading(dtv);
            ...
            sb.Append(LeftAll + " SLOT CONTRACT REF#  </ td>");
            sb.Append(LeftAll + " SLOT OPERATOR </td>");
            sb.Append(LeftAll + " PORT OF LOADING  </ td>");
            sb.Append(LeftAll + " PORT OF DISCHARGE   </td>");
           
            sb.Append(LeftAll + " EFFECTIVEDATE</td>");
            //sb.Append(LeftAll + " VALID TO</td>");
            sb.Append(LeftAll + " SERVICE NAME</td>");
            sb.Append(LeftAll + " SLOT TERM</td>");
            ...
            for (int i = 0; i < dtx.Rows.Count; i++)
            {
                ...
                sb.Append(ContentLeft + dtx.Rows[i]["SLOTCONTRACTREF"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["SLOT OPERATOR"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["POL"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["POD"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["EFFECTIVEDATE"].ToString() + "</td>");
               // sb.Append(ContentLeft + dtx.Rows[i]["Valid To"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["Service Name"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["SLOT Term"].ToString() + "</td>");
            ...
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment;filename=SLOTUploadingReport" + System.DateTime.Now + ".xls");
            Response.Charset = "";
