...
{
    public class VendorInvoicePDFController : Controller
    {
        SalesManager Manag = new SalesManager();
        // GET: VendorInvoicePDF
        public ActionResult Index()
        {
            return View();
        }

        public void SlotBillingPDFValues(string ID)
        {

            DataTable dtv = GetSlotBillingPDFValues(ID);
            //if (dtv.Rows.Count > 0)
            //{
                ...
                tbllogo.AddCell(cell);
                ///----/////
                cell = new Cell(new Phrase("Oceanus Container Lines Pte.Ltd", new Font(Font.HELVETICA, 14, Font.BOLD, Color.BLACK)));
                cell.BorderWidth = 0;
                tbllogo.Alignment = Element.ALIGN_LEFT;
                //cell.Colspan = 2;
                tbllogo.AddCell(cell);

                cell = new Cell(new Phrase("SLOT BILLING INVOICE ", new Font(Font.HELVETICA, 14, Font.BOLD, Color.BLACK)));
                cell.BorderWidth = 0;
        ...
        public DataTable GetSlotBillingPDFValues(string RID)
        {
            string _Query = " ";
            return Manag.GetViewData(_Query, "");
        }

        public DataTable GetSlotBillingDtls(string RID)
        {
            string _Query = "";
            return Manag.GetViewData(_Query, "");
        }
        ...
        public ActionResult PortBillInvoicePDF(string idv)
        {
            BindPortBillInvPDF(idv);
            return View();
        }

        public void BindPortBillInvPDF(string idv)
        {
        ...
        {
            string _Query = "select Id, InvoiceNo, convert(varchar, invDate, 23) as InvDate,PartyName,InvTotal,PartyID, BLID,BranchID,TaxNo,Address,StateCode,InvTax,InvAmount,intBank,CurrencyID,(select top(1) GSTIN from NVO_CustomerMaster Inner Join NVO_CusBranchLocation on NVO_CusBranchLocation.CustomerID = NVO_CustomerMaster.ID where NVO_CustomerMaster.ID = PartyID) as GSTIN," +
              " (select top(1) AgencyName from NVO_AgencyMaster where ID = NVO_InvoicevenBilling.AgentID ) as Agency," +
              " ( select top(1) UserName from NVO_UserDetails where ID = NVO_InvoicevenBilling.UserID ) as CreatedBy from NVO_InvoicevenBilling where NVO_InvoicevenBilling.ID=" + InvID;
            return Manag.GetViewData(_Query, "");
        }
