...
{
    public class TDRMultiPortController : Controller
    {
        ...
        //public ActionResult ExportTerminalMultiPortPDF(string VesVoyID, string PODID, string TSPORTID, string AgentID, string VesselOpID, string AgencyID, string NextPortID)
        //{
        //    BindMultiTerminalPDF(VesVoyID, PODID, TSPORTID, AgentID, VesselOpID, AgencyID, NextPortID);
        //    return View();
        //}


        public FileResult ExportTerminalMultiPortPDFold(string VesVoyID, string PODID, string TSPORTID, string AgentID, string VesselOpID, string AgencyID, string NextPortID)
        {
            string FileHidpath = "";
            ...
            if (_dtv.Rows.Count > 0)
            {
                ...
                cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, " :  " + "GLOBAL NETWORK LINES PTE LTD", 130, 340, 0);
                ...
                for (int i = 1; i < _dtv.Rows.Count; i++)
                {
                    cb.MoveTo(10, 370);
                    cb.LineTo(830, 370);
                    cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, _dtv.Rows[i]["NextPortCol"].ToString(), 15, columnrows, 0);
                    cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, _dtv.Rows[i]["NextPortETACol"].ToString(), 450, columnrows, 0);
                    cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, " :  " + _dtv.Rows[i]["POL"].ToString(), 130, columnrows, 0);
                    cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, " :  " + _dtv.Rows[i]["ETA"].ToString(), 570, columnrows, 0);
                    columnrows -= 15;
                    rowscol++;
                }
                cb.MoveTo(10, 355);
        ...
        public void  ExportTerminalMultiPortPDF(string VesVoyID, string PODID, string TSPORTID, string AgentID, string VesselOpID, string AgencyID, string NextPortID)
        {
        ...
        public DataTable GetTerminalDepPdfValues(string VesVoy, string AgencyID, string NextPortID)
        {
          ...
          " (select top(1) PortName from NVO_PortMainMaster where ID = VR.PortID) as POL, (select top(1) TerminalName from NVO_VoyageRoute inner join NVO_TerminalMaster On NVO_TerminalMaster.ID = NVO_VoyageRoute.TerminalID where NVO_VoyageRoute.VoyageID = V.ID " +
           " order by NVO_VoyageRoute.RID asc) as TerminalName, CASE WHEN ROW_NUMBER() OVER(ORDER BY VR.RID) = 1 THEN 'NextPort'  WHEN ROW_NUMBER() OVER(ORDER BY VR.RID) = 2 THEN 'NextPort1' " +
            ...
            return RegMng.GetViewData(strWhere + " ORDER BY VR.RID ", "");
        }
