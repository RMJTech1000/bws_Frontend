...
{
    public partial class CntrMasterUpload : System.Web.UI.Page
    {
        static string FolderPath = HttpRuntime.AppDomainAppPath + "UploadFolder//ContainerMasterUpload//";
        ...
        protected void btnfileUploading_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string message1 = "Cntr Master file is missing !!!";
        ...
        private void Import_To_Grid(string FilePath, string Extension, string UserID)
        {
            ...
            DataTable dtx = Manag.InsertCntrMasterExcelUploading(dtv, UserID);
            ...
            sb.Append(LeftAll + " CNTRSIZETYPE </td>");
            sb.Append(LeftAll + " ISOCODE  </td>");
            sb.Append(LeftAll + " GRADE   </td>");
            sb.Append(LeftAll + " CUBICCAPACITY  </ td>");
            sb.Append(LeftAll + " GRWEIGHT   </ td>");
            sb.Append(LeftAll + " NETWEIGHT  </ td>");
            sb.Append(LeftAll + " TAREWEIGHT  </ td>");
            sb.Append(LeftAll + " MFGDATE   </ td>");
            sb.Append(LeftAll + " BOXOWNER   </ td>");
            sb.Append(LeftAll + " LEASEPARTNER  </ td>");
            sb.Append(LeftAll + " LEASETERM  </ td>");
            sb.Append(LeftAll + " REFERENCE</ td>");
            sb.Append(LeftAll + " CNTRSTATUS </ td>");
            ...
            for (int i = 0; i < dtx.Rows.Count; i++)
            {
                ...
                sb.Append(ContentLeft + dtx.Rows[i]["CNTRSIZETYPE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["ISOCODE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["GRADE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["CUBICCAPACITY"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["GRWEIGHT"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["NETWEIGHT"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["TAREWEIGHT"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["MFGDATE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["BOXOWNER"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["LEASEPARTNER"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["LEASETERM"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["REFERENCE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["CNTRSTATUS"].ToString() + "</td>");
            ...
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment;filename=CntrMasterUploadReport" + System.DateTime.Now + ".xls");
            Response.Charset = "";
