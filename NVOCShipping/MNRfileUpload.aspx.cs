...
{
    public partial class MNRfileUpload : System.Web.UI.Page
    {
        static string FolderPath = HttpRuntime.AppDomainAppPath + "//MNRUpload//";
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            ...
            DataTable dtx = Manag.InsertMNRExcelUploading(dtv, Request.QueryString["ID"].ToString());
            ...
            sb.Append(LeftAll + " DM CODE  </ td>");
            sb.Append(LeftAll + " REPAIR TYPE </td>");
            sb.Append(LeftAll + " COMPONENT CODE </ td>");
            sb.Append(LeftAll + " LENGTH(CM)  </td>");

            sb.Append(LeftAll + " WIDTH(CM)</td>");
            sb.Append(LeftAll + " LOCATION</ td>");
            sb.Append(LeftAll + " LAB HRS</ td>");
            sb.Append(LeftAll + " LAB COST</ td>");
            sb.Append(LeftAll + " MAT COST</ td>");
            ...
            for (int i = 0; i < dtx.Rows.Count; i++)
            {
                ...
                sb.Append("<tr>");
                sb.Append(ContentLeft + dtx.Rows[i]["S#NO"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["DM CODE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["REPAIR TYPE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["COMPONENT CODE"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["LENGTH(CM)"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["WIDTH(CM)"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["LOCATION"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["LAB HRS"].ToString() + "</td>");
                sb.Append(ContentLeft + dtx.Rows[i]["MAT COST"].ToString() + "</td>");
